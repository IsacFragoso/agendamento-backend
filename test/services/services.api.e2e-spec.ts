...
describe('Services API', () => {
  ...
  it('passes provider identity to service management', async () => {
    ...
    await request(context.app.getHttpServer()).post('/servicos').send({ id_prestador: 12 }).expect(201);
  ...
  it('restricts category management to administrators', async () => {
    await request(context.app.getHttpServer()).post('/categorias').send({ nome: 'Beleza' }).expect(403);
    context.setUser({ sub: 1, tipo_conta: 'ADMIN' });
    await request(context.app.getHttpServer()).post('/categorias').send({ nome: 'Beleza' }).expect(201);
  });
});